...
use combine::{any, any_num, opt, Parse, StructuredLexer};
...
impl<'a> Parser<'a> {
    ...
    pub fn number(&mut self) -> PResult<'a, Spanned<Num>> {
        log::debug!("Parsing a number");
        let (t, num) = any_num.parse_next(&mut self.lexer)?;
        Ok(Spanned::new(Num { value: num }, t.span()))
    }
}
